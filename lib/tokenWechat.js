...
var tokenWechat = function() {
  ...
  function freshToken() {
    ...
    if (globalInfo.token.validTime - currentTimestamp < CONFIG.VALID_TIME_LIMIT) {
      freshTokenFromDb();
    } else if (globalInfo.jsapiTicket.validTime - currentTimestamp < CONFIG.VALID_TIME_LIMIT) {
      getJsapiTicket();
    }
  };
  ...
  function freshTokenFromDb() {
    dbPool.query("select * from tbl_wechat_tokens where appid=?", [CONFIG.WECHAT.APPID], function(err, rows, fields) {
      ...
      if (rows[0].validTime - currentTimestamp < CONFIG.VALID_TIME_LIMIT) {
