...
const { MongoSession } = require('mongodb-pool')
...
module.exports.findDocuments = function(options, callback) {
  ...
  var collection = db.collection(options.collection);
  // Find some documents
  collection.find(options.condition).sort(options.sort).toArray(function(err, docs) {
...
module.exports.findOneDocumentByFilter = function(collectionName, filter, options, callback) {
  ...
  var collection = db.collection(collectionName);
  // Find some documents
  collection.findOne(filter, options, function(err, docs) {
    if (err) { logger.error('findOneDocumentById', err) }
    callback(docs);
  });
}
