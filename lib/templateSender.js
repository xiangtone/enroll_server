...
function processApplyUserName(options) {
  if (options.activity.applys[options.targetIndex].displayNickName == options.activity.applys[options.targetIndex].wechatUserInfo.nickname) {
    return options.activity.applys[options.targetIndex].displayNickName
  } else {
    return options.activity.applys[options.targetIndex].displayNickName + '[' + options.activity.applys[options.targetIndex].wechatUserInfo.nickname + ']'
  }
}
module.exports.sendApply = function(options) {
  init(function() {
    ...
    var msg = {
      ...
      "url": pu.manageApplyUrl(options.activity._id),
      // "miniprogram": {
      //   "appid": "xiaochengxuappid12345",
      //   "pagepath": "index?foo=bar"
      // },
      "data": {
        "first": {
          "value": processApplyUserName(options) + '报名参加活动',
          "color": "#173177"
        },
        "keyword1": {
          "value": options.activity.activityTitle,
...
module.exports.sendCancel = function(options) {
  init(function() {
    if (options.activity.notifySwitch) {
      var activityDateTime = new Date(options.activity.activityDateTime)
      var msg = {
        "touser": options.activity.founderOpenId,
        "url": pu.viewUrl(options.activity._id),
        ...
        "data": {
          "first": {
            "value": processApplyUserName(options) + '取消了参加您' + pu.formatDateToDay(options.activity.activityDateTime) + '活动的申请',
            "color": "#173177"
          },
          "keyword1": {
            "value": options.activity.activityTitle,
